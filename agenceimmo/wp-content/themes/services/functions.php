...
if ( !get_user_meta( $user_id, 'services_notice_dismissed' ) && current_user_can( 'manage_options' ) )
echo '<div class="notice notice-info"><p>' . __( '<big><strong>Thank You!</strong> For using Services. If you need more features and support, you can <a href="https://serviceswp.co/#choices" class="button-primary" target="_blank">Upgrade to Pro</a>.</big>', 'services' ) . '<a href="?notice-dismiss" class="alignright">Dismiss</a></p></div>';
}
add_action( 'admin_init', 'services_notice_dismissed' );
function services_notice_dismissed() {
$user_id = get_current_user_id();
if ( isset( $_GET['notice-dismiss'] ) )
add_user_meta( $user_id, 'services_notice_dismissed', 'true', true );
}
...
add_action( 'wp_body_open', 'services_skip_link', 5 );
function services_skip_link() {
echo '<a href="#content" class="skip-link screen-reader-text">' . esc_html__( 'Skip to the content', 'services' ) . '</a>';
}
