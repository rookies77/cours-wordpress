...
( function( $ ) {
	...
	if ( $.isFunction( $.fn.owlCarousel ) ) {
		//News Ticker
		var ntOptions = {
			rtl:traveloreOptions.rtl ? true : false,
			autoHeight:true,
			items:1,
			nav: false,
			dots: false,
			...
			loop: true,
			navText: [traveloreOptions.iconNavPrev,traveloreOptions.iconNavNext]
		};
	...
	$( function() {
        ...
        if ( $.isFunction( $.fn.fitVids ) ) {
            $( '.travelore-video-section .hentry' ).fitVids();
        }
	});
	...
	function initMainNavigation( container ) {
		...
		var dropdownToggle = $( '<button />', { 'class': 'dropdown-toggle', 'aria-expanded': false })
			.append( traveloreOptions.screenReaderText.icon )
			.append( $( '<span />', { 'class': 'screen-reader-text', text: traveloreOptions.screenReaderText.expand }) );
		...
		container.find( '.current-menu-ancestor > button' )
			...
			.text( traveloreOptions.screenReaderText.collapse );
		...
		container.find( '.dropdown-toggle' ).click( function( e ) {
			...
			// jscs:enable
			screenReaderSpan.text( screenReaderSpan.text() === traveloreOptions.screenReaderText.expand ? traveloreOptions.screenReaderText.collapse : traveloreOptions.screenReaderText.expand );
		} );
	}
	...
	( function() {
		...
		menuToggle.on( 'click.travelore', function() {
	...
	( function() {
		...
		function toggleFocusClassTouchScreen() {
			if ( window.innerWidth >= 1024 ) {
				$( document.body ).on( 'touchstart.travelore', function( e ) {
					if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
						$( '.main-navigation li' ).removeClass( 'focus' );
					}
				} );
				siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.travelore', function( e ) {
				...
				siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.travelore' );
			}
		}

		if ( 'ontouchstart' in window ) {
			$( window ).on( 'resize.travelore', toggleFocusClassTouchScreen );
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find( 'a' ).on( 'focus.travelore blur.travelore', function() {
	...
	$('.travelore-tabbed-cats li a').on('click', function(e){
		e.preventDefault();
	...
	$( document.body ).on( 'post-load', function () {
		var $container = $('.header-highlights-content-wrapper');
