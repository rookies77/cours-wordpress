<?php
/**
 * Travelore Theme Options Sanitization Functions
 *
 * @package Travelore
 */
 ...
 */
function travelore_sanitize_select( $input, $setting ) {
 ...
 */
function travelore_sanitize_checkbox( $checked ) {
    // Boolean check.
 ...
 */
function travelore_sanitize_post( $input ) {
 ...
 */
function travelore_sanitize_image( $image, $setting ) {
    /*
 ...
 * @since Travelore 1.0
 */
function travelore_sanitize_category_list( $input ) {
...
/**
 * Sanitizes tag list in slider
 * @param  $input entered value
 * @return sanitized output
 *
 * @since Travelore 1.0
 */
function travelore_sanitize_tag_list( $input ) {
    if ( '' != $input ) {
        if( in_array( 0, $input ) ) {
            return '0';
        }
        ...
        foreach ( $tags as $tag )
            $tag_list  =   array_merge( $tag_list, array( $tag->term_id ) );

        if ( count( array_intersect( $input, $tag_list ) ) == count( $input ) ) {
            return $input;
        }
        else {
            return '';
 ...
 *                    the setting default.
 */
function travelore_sanitize_number_range( $number, $setting ) {
